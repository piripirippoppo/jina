...
#STEP1
weather <- read_html("https://www.spaceweatherlive.com/en/solar-activity/top-50-solar-flares") %>%
...
#STEP2
weather <- weather[c(1:7)] # chop "movie" column

weather <- weather %>% 
  type_convert(col_types = cols(start_datetime = col_datetime(format = "%F %T"))) %>%
